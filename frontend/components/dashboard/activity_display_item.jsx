...
class ActivityDisplayItem extends React.Component {
  ...
  render() {
    // const { distance, elevation, time, description, name, id, created_at } = this.props.activity;
        ...
        <div className="activity-info">
          ...
          <section>
            <header className="activity-feed-item-header">
              ...
              <label>
                {this.dateConverter(this.props.activity.updated_at)}
              </label>
            </header>
            <div className="activity-feed-item-name">
              <Link
                className="route-link"
                to={`/${this.props.type}/${this.props.activity.id}`}
              >
            ...
            <div className="activity-feed-item-stats">
              <section>
                ...
                <h3>{this.props.activity.distance} mi</h3>
              </section>
              <section>
                <label>Elevation</label>
                <h3>{this.props.activity.elevation} ft</h3>
              </section>
              <section>
                <label>Time</label>
                <h3>{this.timeConverter(this.props.activity.time)}</h3>
              </section>
            </div>
          </section>
        </div>
